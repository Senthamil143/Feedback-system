...
import { getTeamMembers, getAvailableEmployees, assignEmployeeToManager, submitFeedback, getFeedbackByManager, getManagerDashboard, updateFeedback, getTags, getFeedbackRequests, getFeedbackForManager, getPendingRequests, createFeedback, addComment, createTag, exportFeedbackToPdf, approveRequest, denyRequest, makeAuthenticatedRequest } from '../utils/api';
...
const ManagerDashboard = ({ user, setUser }) => {
  ...
  const fetchData = async () => {
    ...
    // Fetch team members
      ...
      const availableData = await getAvailableEmployees(user.id);
      setAvailableEmployees(availableData);
      
    // Fetch feedbacks
      const feedbackData = await getFeedbackForManager();
      setFeedbacks(feedbackData);
      
    // Fetch dashboard data
      const dashboardData = await getManagerDashboard(user.id);
  ...
  }, [user?.id]); // Using user.id directly makes the dependency stable.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await submitFeedback({ ...form, manager_id: user.id });
    ...
    <div>
      ...
      {feedbacks.length === 0 ? (
                ...
                <div style={{ display: 'grid', gap: '15px' }}>
                  {feedbacks.map(fb => {
                    // Check if feedback has been acknowledged
                    const hasAcknowledgement = fb.acknowledgements && fb.acknowledgements.length > 0;
                    const acknowledgment = hasAcknowledgement ? fb.acknowledgements[0] : null;
                    ...
                    return (
                      <div key={fb.id} style={{ 
                        ...
                        {hasAcknowledgement && acknowledgment && (
                          <div style={{ 
                            marginTop: '10px', 
                            padding: '10px', 
                            ...
                            <strong>Acknowledged on:</strong> {new Date(acknowledgment.acknowledged_at + 'Z').toLocaleString('en-US', {
                              year: 'numeric',
                              month: 'long',
                              day: 'numeric',
                              hour: '2-digit',
                            ...
                            {acknowledgment.comment && (
                              <div style={{ marginTop: '5px' }}>
                                <strong>Comment:</strong> <ReactMarkdown>{acknowledgment.comment}</ReactMarkdown>
                              </div>
                            )}
                          </div>
                        )}
            ...
            {feedbackRequests.length > 0 && (
              <div style={{ marginBottom: '40px', padding: '20px', backgroundColor: '#fffbe6', border: '1px solid #ffe58f', borderRadius: '8px' }}>
                  ...
                  {feedbackRequests.map(req => (
                      <div key={req.id} style={{ borderBottom: '1px solid #eee', padding: '15px 0' }}>
                          ...
                          <button 
                              onClick={() => setForm({ ...form, employee_id: req.employee_id })}
